...
export const Dashboard: React.FC<DashboardProps> = ({
  ...
  const trendPoints = useMemo(() => {
    const points = [
      totalBalanceUSD * 0.9,
      totalBalanceUSD * 0.94,
      totalBalanceUSD * 0.91,
      totalBalanceUSD * 0.96,
      totalBalanceUSD * 0.98,
      totalBalanceUSD,
    ];
  ...
  const expenseStructure = useMemo(() => {
    ...
    const total =
      expenses.reduce((acc, t) => acc + t.normalizedAmountUSD, 0) || 1;
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6 md:mt-6">
          ...
          <div className="md:col-span-5 lg:col-span-4 flex flex-col gap-6">
            ...
            <div className="px-4 md:px-0">
              <div className="bg-theme-surface rounded-3xl p-8 relative overflow-hidden active:scale-[0.98] transition-transform duration-200 group shadow-2xl shadow-black/50 border border-white/5">
                {/* Top Actions */}
                <div className="absolute top-6 right-6 flex gap-2 z-20">
                  <button
                    onClick={toggleCurrency}
                    className="p-2 rounded-full bg-black/20 hover:bg-black/40 text-theme-secondary transition-colors"
                  >
                ...
                >
                  ...
                  <div className="flex flex-col gap-2 relative z-10 mb-8">
                    <h1 className="text-5xl font-bold tracking-tight text-theme-primary transition-all">
                      {isBalanceVisible ? (
                        <>
                          <span className="text-3xl align-top opacity-60 mr-1">
                            {symbolMain}
                          </span>
            ...
            <div className="px-4 md:px-0">
              <div className="flex items-center justify-between mb-2">
                ...
                <button
                  ...
                  className="text-theme-brand text-xs font-bold"
                >
                  {t("manage")}
                </button>
              </div>
            ...
            <div className="flex justify-between px-6 md:px-0 gap-4">
              {[
                ...
                >
                  <div
                    className={`w-12 h-12 rounded-xl ${action.color} border flex items-center justify-center shadow-lg`}
                  >
                    {action.icon}
                  </div>
            ...
            >
              <div>
                <h3 className="text-sm font-bold text-theme-secondary uppercase tracking-wider mb-1 flex items-center gap-2">
                  ...
                  <ArrowUpRight
                    size={14}
                    className="opacity-0 group-hover:opacity-100 transition-opacity text-theme-brand"
                  />
                </h3>
                <p className="text-xs text-theme-secondary">
                  {t("last30DaysExpenses")}
                </p>
              </div>
              <div className="flex items-center gap-4">
                ...
                {expenseStructure.length > 0 ? (
                  <>
                    ...
                    <div
                      className="w-16 h-16 rounded-full relative overflow-hidden flex items-center justify-center shadow-lg bg-black/50"
                      style={{
                                        ...
                                        ${expenseStructure
                                          .map((item, i, arr) => {
                                            ...
                                            if (item.color.includes("red"))
                                              color = "#ef4444";
                                            if (item.color.includes("orange"))
                                              color = "#f97316";
                                            if (item.color.includes("amber"))
                                              color = "#f59e0b";
                                            if (item.color.includes("yellow"))
                                              color = "#eab308";
                                            if (item.color.includes("lime"))
                                              color = "#84cc16";
                                            if (item.color.includes("green"))
                                              color = "#22c55e";
                                            if (item.color.includes("emerald"))
                                              color = "#10b981";
                                            if (item.color.includes("teal"))
                                              color = "#14b8a6";
                                            if (item.color.includes("cyan"))
                                              color = "#06b6d4";
                                            if (item.color.includes("sky"))
                                              color = "#0ea5e9";
                                            if (item.color.includes("blue"))
                                              color = "#3b82f6";
                                            if (item.color.includes("purple"))
                                              color = "#a855f7";
                                            if (item.color.includes("fuchsia"))
                                              color = "#d946ef";
                                            if (item.color.includes("pink"))
                                              color = "#ec4899";
                                            if (item.color.includes("rose"))
                                              color = "#f43f5e";
                          ...
                          return (
                            <div key={date}>
                              ...
                              <div className="flex flex-col gap-2 mt-1">
                                ...
                                <div
                                  key={transaction.id}
                                  className="flex items-center justify-between p-3 rounded-xl hover:bg-white/5 transition-colors group relative pr-14"
                                >
                                  <div className="flex items-center gap-4">
                                    <div
